...
def build_basic_inference_model(model_path):
    ...
    # encoder_embedding
    encoder_embedding = model.get_layer(index=1)(encoder_inputs)
    # get encoder states
    encoder_lstm_1_out, *encoder_states_1 = model.get_layer(index=2)(encoder_embedding)
    encoder_lstm_2_out, *encoder_states_2 = model.get_layer(index=4)(encoder_lstm_1_out)
    _, *encoder_states_3 = model.get_layer(index=6)(encoder_lstm_2_out)
    encoder_model = Model(encoder_inputs, encoder_states_3)
    ...
    # decoder embedding
    decoder_embedding = model.get_layer(index=5)(decoder_inputs)
    # get rnn outputs and decoder states
    rnn_outputs, *decoder_states = model.get_layer(index=7)(decoder_embedding, initial_state=decoder_states_inputs)
    decoder_outputs = model.get_layer(index=8)(rnn_outputs)
    decoder_model = Model([decoder_inputs] + decoder_states_inputs, [decoder_outputs] + decoder_states)

    return encoder_model, decoder_model


encoder_model, decoder_model = build_basic_inference_model('model/word_triple_lstm_epo_30.h5')
