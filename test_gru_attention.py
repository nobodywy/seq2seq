...
from models.attention_cell import AttentionRNNCell
...
def build_basic_model():
    ...
    rnn_encoded, *encoder_states = LSTM(latent_dim,  return_sequences=True, return_state=True, name='encoder_lstm')(encoder_embedding)


    decoder_inputs = Input(shape=(None,), name='decoder_inputs')
    decoder_embedding = target_emedding_layer(decoder_inputs)

    attentionCell = AttentionRNNCell(units=latent_dim, encoder_ts = max_encoder_seq_length,encoder_latdim=latent_dim)
    Decoder_layer = RNN(attentionCell,return_sequences=True,return_state=True)
    decoder_outputs,decoder_states = Decoder_layer(decoder_embedding, encoder_states[0], constants = rnn_encoded)

    decoder_dense = Dense(num_decoder_tokens, activation='softmax', name='decoder_dense')
    decoder_outputs = decoder_dense(decoder_outputs)

    basic_model = Model(inputs=[encoder_inputs,decoder_inputs], outputs=[decoder_outputs])
...
#basic_model.load_weights('./model/attention_model_gpu_epo_1_weights.h5')
...
def build_basic_inference_model(model_path):
    basic_model.load_weights(model_path)

    # encoder
    encoder_inputs = Input(shape=(None,))
    # encoder_embedding
    encoder_embedding = basic_model.get_layer(index=2)(encoder_inputs)
    # get encoder states
    rnn_encoded, *encoder_states = basic_model.get_layer(index=4)(encoder_embedding)
    encoder_model = Model(encoder_inputs, [rnn_encoded] + [encoder_states[0]])
    ...
    encoder_H = Input(shape=(max_encoder_seq_length, latent_dim))
    decoder_states_inputs = [decoder_state_h]

    # decoder embedding
    decoder_embedding = basic_model.get_layer(index=3)(decoder_inputs)

    # get rnn outputs and decoder states
    atttention_gru = basic_model.get_layer(index=5)

    rnn_outputs, decoder_states_output = atttention_gru(decoder_embedding, decoder_states_inputs,constants=encoder_H)  # encoder_states[0],rnn_encoded

    decoder_outputs = basic_model.get_layer(index=6)(rnn_outputs)
    decoder_model = Model([decoder_inputs] + decoder_states_inputs + [encoder_H], [decoder_outputs] + [decoder_states_output] )

    return encoder_model, decoder_model


encoder_model, decoder_model = build_basic_inference_model('./model/attention_model_gpu_epo_10_weights.h5')
...
def decode_sequence(input_seq,encoder_model,decoder_model):
    # 对输入序列进行编码，并输出其隐藏状态
    encoder_H, decoder_init_states = encoder_model.predict(input_seq)
    states_value = [decoder_init_states]
    # 创建空的输出序列
    ...
    while not stop_condition:
        output_tokens, h = decoder_model.predict([target_seq] + states_value + [encoder_H] )
